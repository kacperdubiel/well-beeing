<template>
    <div style="text-align: start;" class="container-fluid">
        ...
        <table v-if="this.userNutritionPlans.length !== 0" style="color: white; text-align: start;" class="table">
            <thead>
                <tr>
                    <th class="col-sm-5" scope="col">Nazwa</th>
                    <th class="col-sm-5" scope="col">Data utworzenia</th>
                    <th class="col-sm-2" scope="col"></th>
                </tr>
            </thead>
            <tbody>
                <tr class="tr-row" v-for="plan in this.userNutritionPlans" :key="plan.id" @click="this.setActualNutritionPlan(plan)" style="cursor: pointer;">
                    <td class="col-sm-5" v-bind:class="{'main' : plan.id === this.actualPlan.id}">{{plan.name}} <font-awesome-icon class="mx-3" v-if="plan.main" :icon="['fa', 'home']"/></td>
                    <td class="col-sm-5" v-bind:class="{'main' : plan.id === this.actualPlan.id}">{{this.$func_global.formatDateTime(plan.generationDate)}}</td>
                    ...
                    <div class="modal-body">
                        <div style="text-align: start;">
                            ...
                            <div style="width: 100%; justify-content: flex-end; display: flex;">
                                <button @click="addEmpyNutritionPlan" style="margin: 2px; slign-self: end;" type="button" class="btn-card-panel-diet">Dodaj plan</button>
                            </div>
