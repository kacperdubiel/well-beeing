<template>
    <div class="container-fluid" style="align-self: center; display: flex; align-items: stretch;">
        <div class="card-container">
            ...
            <div class="card-elem-container">
                ...
                <div class="col-lg-8">
                    <ailment-row-component v-for="ailment_elem in ailments"
                                           ...
                                           class="card-elem-container-row"
                                           @show:modal="showModal"></ailment-row-component>
                </div>
            </div>
            <!--<div class="card-elem-container">
                <div class="col-lg-8">
            ...
            <div v-if="!this.dieticianView" style="justify-content: flex-end; flex-direction: row; display: flex;">
                <button class="btn-card-panel-diet" data-bs-target="#profileCardFormModal" data-bs-toggle="modal">Edytuj
                    dane
                </button>
            </div>
            <div id="ailmentModal" aria-hidden="false" aria-labelledby="ailmentModalLabel" class="modal fade"
                 tabindex="-1">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-ailment-content">
                        <h3>{{ modalData.name }}</h3>
                        <p style="font-size: 10pt; color: grey;">
                            {{ this.$func_global.mapAilmentType(modalData.type) }}</p>
                        <span style="text-align: left;">{{ modalData.description }}</span>
                    </div>
                </div>
            </div>
            <profile-card-form-component v-if="!this.dieticianView" :ailmentsSource="this.allAilments"
                                         :chosenAilmentsSource="this.ailments" :formCardData="dataToObject()"
                                         @save:card="saveEditedCard"></profile-card-form-component>
        </div>
    </div>
</template>
